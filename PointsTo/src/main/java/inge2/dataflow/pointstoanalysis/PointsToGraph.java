...
public class PointsToGraph {
    ...
    public Set<Node> getReachableNodesByField(Node node, String fieldName) {
         ...
         for (Axis axis : axis) {
             if (axis.leftNode == node && axis.fieldName.equals(fieldName)) {
    ...
    public void union(PointsToGraph in) {
         ...
         axis.addAll(in.axis);
         for (String variable : in.mapping.keySet()) {
             if (mapping.containsKey(variable)) {
                 Set<Node> unionOfNodeSets = new HashSet<>();
                 unionOfNodeSets = this.getNodesForVariable(variable);
                 unionOfNodeSets.addAll(in.getNodesForVariable(variable));
                 this.setNodesForVariable(variable, unionOfNodeSets);
             } else this.setNodesForVariable(variable, in.getNodesForVariable(variable));
         }
    }
}
