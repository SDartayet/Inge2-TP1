...
public class PointsToAnalysis extends ForwardFlowAnalysis<Unit, PointsToGraph> {
    ...
    public boolean mayAlias(String leftVariableName, String fieldName, String rightVariableName) {
        Set<Node> nodesPointedToByLeft = lastPointsToGraph.getNodesForVariable(leftVariableName);
        Set<Node> graphNodes = new HashSet<>();
        for (Node node : nodesPointedToByLeft) {
            graphNodes.addAll(lastPointsToGraph.getReachableNodesByField(node, fieldName));
        }
        Set<Node> nodesPointedToByRight = lastPointsToGraph.getNodesForVariable(rightVariableName);
        return graphNodes.stream().anyMatch(nodesPointedToByRight::contains);
    }
}
